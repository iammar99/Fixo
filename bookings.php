...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        ...
        <div class="flex border-b border-gray-200 mt-6 overflow-x-auto" id="statusTabs">
            <button data-status="all" class="px-6 py-3 border-b-2 border-fixo-orange text-fixo-orange font-medium whitespace-nowrap">
                All (<?php echo count($recent_bookings); ?>)
            </button>
            <button data-status="pending" class="px-6 py-3 text-gray-600 hover:text-gray-900 font-medium whitespace-nowrap">
                Pending (<?php echo count(array_filter($recent_bookings, fn($b) => $b['status'] === 'pending')); ?>)
            </button>
            <button data-status="accepted" class="px-6 py-3 text-gray-600 hover:text-gray-900 font-medium whitespace-nowrap">
                Accepted (<?php echo count(array_filter($recent_bookings, fn($b) => $b['status'] === 'accepted')); ?>)
            </button>
            <button data-status="in_progress" class="px-6 py-3 text-gray-600 hover:text-gray-900 font-medium whitespace-nowrap">
                In Progress (<?php echo count(array_filter($recent_bookings, fn($b) => $b['status'] === 'in_progress')); ?>)
            </button>
            <button data-status="completed" class="px-6 py-3 text-gray-600 hover:text-gray-900 font-medium whitespace-nowrap">
                Completed (<?php echo count(array_filter($recent_bookings, fn($b) => $b['status'] === 'completed')); ?>)
            </button>
            <button data-status="cancelled" class="px-6 py-3 text-gray-600 hover:text-gray-900 font-medium whitespace-nowrap">
    ...
    <div id="searchResultsInfo" class="mb-4 p-3 bg-blue-50 rounded-lg hidden">
        <p class="text-blue-800">
            ...
            <button onclick="clearSearch()" class="ml-2 text-blue-600 hover:text-blue-800 underline">Clear search</button>
        </p>
    </div>
                ...
                <div class="p-6">
                    ...
                    <div class="flex justify-between items-start mb-6">
                        ...
                        <div class="flex space-x-2">
                            ...
                            <?php elseif ($booking['status'] == 'accepted'): ?>
                                <button onclick="updateBookingStatus(<?php echo $booking['id']; ?>, 'in_progress')"
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="bg-gray-50 rounded-xl p-4">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 rounded-full bg-fixo-orange flex items-center justify-center text-white font-bold mr-3">
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="flex-1 flex items-center">
                                ...
                                <div class="ml-3">
                                    ...
                                    <p class="text-sm text-gray-500"><?php echo (new DateTime($booking['created_at']))->format('M d, h:i A'); ?></p>
                                </div>
                            </div>
                            ...
                            <div class="flex-1 flex items-center">
                                <div class="w-8 h-8 rounded-full 
                                    <?php 
                                    if (in_array($booking['status'], ['completed', 'in_progress'])) echo 'bg-blue-500';
                                    elseif ($booking['status'] === 'accepted') echo 'bg-gray-300';
                                    else echo 'bg-gray-200';
                                    ?> 
                                    flex items-center justify-center text-white">
                                    <i class="fas 
                                        <?php 
                                        if (in_array($booking['status'], ['completed', 'in_progress'])) echo 'fa-tools';
                                        else echo 'fa-clock';
                                        ?> 
                                        text-sm"></i>
                                </div>
                                <div class="ml-3">
                                    ...
                                    <p class="text-sm text-gray-500">
                                        ...
                                        elseif ($booking['status'] === 'in_progress') echo 'In Progress';
                                        elseif ($booking['status'] === 'accepted') echo 'Scheduled';
...
<div id="bookingDetailsModal" class="hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
...
<script>
    ...
    function updateBookingStatus(bookingId, status) {
        if (confirm('Are you sure you want to update this booking status?')) {
            fetch('/Proccessing_pages/Dashboard/update_booking_status.php', {
                method: 'POST',
                headers: {
    ...
    function viewBookingDetails(bookingId) {
        fetch('/Proccessing_pages/Dashboard/get_booking_details.php?id=' + bookingId)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    document.getElementById('modalCustomerName').textContent = data.customer_name || 'N/A';
                    document.getElementById('modalCustomerPhone').textContent = data.customer_phone || 'N/A';
                    document.getElementById('modalCustomerEmail').textContent = data.customer_email || 'N/A';
                    document.getElementById('modalCustomerAddress').textContent = data.customer_address || 'N/A';
                    document.getElementById('modalProblemType').textContent = data.problem_type || 'N/A';
                    document.getElementById('modalProblemDescription').textContent = data.problem_description || 'N/A';
                    document.getElementById('modalBookingDate').textContent = data.created_at || 'N/A';
                    document.getElementById('modalBookingStatus').textContent = data.status || 'N/A';
    ...
    function saveBookingNotes() {
        alert('Notes saved! (This would save to database)');
        closeBookingModal();
    }
